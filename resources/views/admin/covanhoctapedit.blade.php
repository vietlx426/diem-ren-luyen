...
@section('content')
  <div class="row">
    <div class="x_panel">
      <div class="x_title">
        <h2>CẬP NHẬT CỐ VẤN HỌC TẬP</h2>
        <div class="clearfix"></div>
      </div>
      <div class="x_content">
        <form method="POST" action="{{route('admin_covanhoctap_update')}}" enctype="multipart/form-data" class="form-horizontal form-label-left">
          {{csrf_field()}}
          <input type="text" name="idcovanhoctap" value="{{isset($coVanHocTap)?$coVanHocTap->id:''}}" class="hidden" hidden="true">
          <div class="row">
              ...
              <select name="cbgv" id="cbgv" class="form-control">
                ...
                @if(isset($dsCanBoGiangVien))
                  @foreach($dsCanBoGiangVien as $canBoGiangVien)
                    <option value="{{$canBoGiangVien->id}}" {{$canBoGiangVien->id == old('cbgv', $coVanHocTap->canbogiangvien_id) ? 'selected="true"' : ''}}>{{$canBoGiangVien->hochulot . ' ' . $canBoGiangVien->ten . ' - Khoa ' . $canBoGiangVien->bomonto->khoa->tenkhoa}}</option>
                  @endforeach
                @endif
              </select>
              @if ($errors->has('cbgv'))
              ...
              <select name="lop" id="lop" class="form-control">
                ...
                @if(isset($dsLop))
                  @foreach($dsLop as $lop)
                    <option value="{{$lop->id}}" {{$lop->id == old('lop', $coVanHocTap->lop_id) ? 'selected="true"' : ''}}>{{$lop->tenlop}}</option>
                  @endforeach
                @endif
              </select>
              @if ($errors->has('lop'))
