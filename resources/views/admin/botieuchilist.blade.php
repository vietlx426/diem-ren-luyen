    ...
    <div class="row">
        <div class="x_panel">
            ...
            <div class="pull-right">
                <a href="{{ route('admin_botieuchi_create') }}" class="btn btn-primary"> <i class="fa fa-plus"></i> <strong> THÊM MỚI </strong> </a>
            </div>
                                    ...
                                    @foreach($dsBoTieuChi as $boTieuChi)
                                        <tr>
                                            ...
                                            <td class="text-center-middle text-center" title="Xem chi tiết tiêu chí">
                                                <a href="{{route('admin_botieuchi_tieuchi_index', ['idbotieuchi'=>$boTieuChi->id])}}" class="btn btn-info"><i class="fa fa-eye"></i></a>
                                            </td>
                                            ...
                                            <td class="text-justify-middle">
                                                @if(count($boTieuChi->hockynamhocbotieuchi) == 0)
                                                    <a href="{{route('admin_botieuchi_edit', ['id'=>$boTieuChi->id])}}" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                                                    <a href="{{route('admin_botieuchi_destroy', ['id'=>$boTieuChi->id])}}" class="btn btn-danger btn-remove"><i class="fa fa-trash"></i></a>
                                                @else
                                                    @if(!App\Http\Controllers\HocKyNamHocBoTieuChiController::hasBangDiemTieuChi($boTieuChi))
                                                        <a href="{{route('admin_botieuchi_edit', ['id'=>$boTieuChi->id])}}" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                                                    @endif
                                                @endif
