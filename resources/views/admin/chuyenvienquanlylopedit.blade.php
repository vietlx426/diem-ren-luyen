...
@section('content')
  <div class="row">
    <div class="x_panel">
      ...
      <div class="x_content">
        <form method="POST" action="{{route('admin_expers_update')}}" enctype="multipart/form-data" class="form-horizontal form-label-left">
          {{csrf_field()}}
          <div class="row">
          ...
          <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 form-group{{$errors->has('cbgv')? ' has-error' : ''}}">
              ...
              <input type="text-center" name="cbgv" hidden="true" class="hidden" value="{{$canBoGiangVien->id}}">
              <input type="text" class="form-control" value="{{$canBoGiangVien->hochulot . ' ' . $canBoGiangVien->ten . ' - ' . $canBoGiangVien->bomonto->khoa->tenkhoa}}" readonly="true">
                      ...
                      <ul class="list-group checked-list-box">
                          @if(isset($dsLop))
                              @foreach($dsLop as $lop)
                                <?php 
                                  $strchk="";
                                  foreach (old('lop', $dsLopDangPhuTrach) as $key => $lopDangPhuTrach) {
                                    if ($lop->id == $lopDangPhuTrach->lop_id)
                                      $strchk = 'checked="true"';
                                  }
                                 ?>
                                  <li class="list-group-item" value="{{$lop->id}}"> <input type="checkbox" id="{{$lop->id}}" name="lop[]" value="{{$lop->id}}" class="flat" {{$strchk}} >
                                  <label for="{{$lop->id}}"> {{$lop->tenlop}} </label> </li>
                              @endforeach
                          @endif
                      </ul>
                  </div>
