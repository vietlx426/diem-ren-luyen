            ...
            <div class="x_content">
                <form action="{{route('admin_tieuchi_minhchung_modulethoigianstore')}}" method="POST" class="form-horizontal form-label-left" enctype="multipart/form-data">
                    ...
                    <div class="row text-center">
                        ...
                        @if(isset($tieuChiModuleThoiGian))
                            @if($tieuChiModuleThoiGian)
                                <a href="{{route('admin_tieuchi_minhchung_modulekekhailylich', ['idhockynamhoc'=>$idHocKy, 'idtieuchi'=>$tieuChi->id, 'idmodule'=>$idModule])}}" class="btn btn-danger"><i class="fa fa-cogs fa-spin"></i><strong> THỰC HIỆN TÍNH ĐIỂM </strong></a>
                            @endif
                        @endif
                    </div>
                </form>
