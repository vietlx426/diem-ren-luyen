...
@section('content')
  <div class="row">
    <div class="x_panel">
      ...
      <div class="x_content">
        <form method="POST" action="{{route('admin_khoahoc_store')}}" enctype="multipart/form-data" class="form-horizontal form-label-left">
          {{csrf_field()}}
          ...
          <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-6 form-group{{ $errors->has('tenkhoahoc') ? ' has-error' : '' }}">
                ...
                <input type="text" id="tenkhoahoc" name="tenkhoahoc" class="form-control" value="{{old('tenkhoahoc')}}" placeholder="Khóa học => CD43 hoặc DH19">
                ...
                <div class="form-control no-border">
                  @if(isset($dsBacDaoTao))
                    @foreach($dsBacDaoTao as $bacDaoTao)
                      &emsp;&emsp;
                      <input type="radio" id="radbacdaotao{{$bacDaoTao->id}}" name="bacdaotao" class="flat" value="{{$bacDaoTao->id}}" {{($bacDaoTao->id == old('bacdaotao')) ? 'checked="true"':''}}> 
          ...
          <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-6 form-group{{ $errors->has('nambatdau') ? ' has-error' : '' }}">
                <label class="control-label">Năm bắt đầu</label>
                  <input type="text" class="form-control" id="nambatdau" name="nambatdau" value="{{old('nambatdau')}}" placeholder="Năm bắt đầu">
            ...
            <div class="col-xs-12 col-sm-6 col-md-6 form-group{{ $errors->has('namketthuc') ? ' has-error' : '' }}">
                <label class="control-label">Năm kết thúc</label>
                  <input type="text" class="form-control" id="namketthuc" name="namketthuc" value="{{old('namketthuc')}}" placeholder="Năm kết thúc">
