...
@section('content')
  <div class="row">
    <div class="x_panel">
      ...
      <div class="x_content">
        <form method="POST" action="{{route('admin_canbogiangvien_store')}}" enctype="multipart/form-data" class="form-horizontal form-label-left">
          {{csrf_field()}}
          ...
          <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-6 form-group{{ $errors->has('macanbogiangvien') ? ' has-error' : '' }}">
                ...
                <input type="text" id="macanbogiangvien" name="macanbogiangvien" class="form-control" value="{{old('macanbogiangvien')}}" placeholder="Mã cán bộ - giảng viên">
          ...
          <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-6 form-group{{ $errors->has('dienthoaicanhan') ? ' has-error' : '' }}">
                ...
                <input type="text" class="form-control pull-right" id="dienthoaicanhan" name="dienthoaicanhan" value="{{old('dienthoaicanhan')}}" placeholder="Số điện thoại">
          ...
          <div class="row">
            <!-- <div class="col-xs-12 col-sm-6 col-md-6 form-group{{ $errors->has('bophancongtac') ? ' has-error' : '' }}">
                ...
                <input type="text" class="form-control pull-right" id="bophancongtac" name="bophancongtac" value="{{old('bophancongtac')}}" placeholder="Bộ phận công tác">
              ...
              <div class="form-control no-border">
                @if(isset($DS_LoaiCBGV))
                  @foreach($DS_LoaiCBGV as $LoaiCBGV)
                    &emsp;
                    <input type="radio" id="loaicbgv{{$LoaiCBGV->id}}" name="loaicbgv" class="minimal-red flat" value="{{$LoaiCBGV->id}}" {{$LoaiCBGV->id == old('loaicbgv') ? 'checked="true"' : ''}}>
              ...
              <select name="khoaphong" id="khoaphong" class="form-control">
                ...
                @if(isset($DS_Khoa))
                  @foreach($DS_Khoa as $Khoa)
                    <option value="{{$Khoa->id}}" {{$Khoa->id == old('khoaphong') ? 'selected="true"' : ''}}>{{$Khoa->tenkhoa}}</option>
                  @endforeach
                @endif
              </select>
              ...
              <select name="bomonto" id="bomonto" class="form-control">
                ...
                @if(isset($DS_BoMon))
                  @foreach($DS_BoMon as $BoMon)
                    <option value="{{$BoMon->id}}" {{$BoMon->id == old('bomonto') ? 'selected="true"' : ''}}>{{$BoMon->tenbomon}}</option>
                  @endforeach
                @endif
              </select>
          ...
          <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12 form-group{{ $errors->has('ghichu') ? ' has-error' : '' }}">
              <div class="form-group">
                ...
                <input type="text" class="form-control pull-right" id="ghichu" name="ghichu" value="{{old('ghichu')}}" placeholder="Ghi chú">
