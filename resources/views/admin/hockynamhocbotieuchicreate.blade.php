...
@section('content')
    <div class="row">
        <div class="x_panel">
            ...
            <div class="x_content">
                <form action="{{route('admin_hockynamhocbotieuchi_store')}}" method="POST" class="form-horizontal form-label-left" enctype="multipart/form-data">
                    {{csrf_field()}}
                    @include('layouts.gentelella-master.blocks.flash-messages')
                    <div class="row">
                        @if(isset($hocKyNamHoc) && $hocKyNamHoc)
                            <input type="text" name='idhockynamhoc' class="hidden" hidden="true" value="{{$hocKyNamHoc->id}}">
                            <div class="col-xs-12 col-sm-12 col-md-10 col-lg-10 col-xl-10 {{$errors->has('hockynamhoc') ? ' has-error' : ''}}">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-9 col-sm-9 col-xs-12">
                                        <input type="text" class="inpmuc form-control" readonly="true" value="{{$hocKyNamHoc->tenhockynamhoc}}">
                                    ...
                                    <div class="col-md-9 col-sm-9 col-xs-12">
                                        ...
                                        @if(isset($dsBoTieuChi))
                                            @foreach($dsBoTieuChi as $boTieuChi)
                                                <input type="radio" class="flat" id="inpbotieuchi{{$boTieuChi->id}}" name="botieuchi" value="{{$boTieuChi->id}}" {{$boTieuChi->id==old('botieuchi')?'checked="true"':''}}>
